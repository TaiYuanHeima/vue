...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //功能函数:选中部分子集
      setchildrentobeselect(arr, key) {
        arr.forEach((v, i) => {
          v.checks = key;
          // v._expanded = key;//选中后展开子项
          if (v.children) {
            this.setchildrentobeselect(v.children, v.checks);
          }
        });
      },
      //是否所有的都被选中
      ...
      toselect(row) {
        ...
        //1、若有子集先让子选中
        if (row.children) {
          this.setchildrentobeselect(row.children, row.checks);
        }
        //2、然后判断是否全选中
    ...
    mounted() {
      this.$nextTick(() => {
        ...
        all.onchange = function(e) {
          ...
          if (all.checked == true) {
            that.setchildrentobeselect(that.formatData, true);
          } else {
            that.setchildrentobeselect(that.formatData, false);
          }
        };
      });
    }
